...
export async function GET(_req: NextRequest, { params }: Ctx) {
  ...
  await connectDB()
  const item = await Topic.findById(params.id).lean()
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  ...
  const item = await Topic.findByIdAndUpdate(params.id, body, { new: true })
  if (!item) return NextResponse.json({ ok: false, error: 'Không tìm thấy.' }, { status: 404 })
  return NextResponse.json({ ok: true, data: item })
}
...
export async function DELETE(_req: NextRequest, { params }: Ctx) {
  ...
  await connectDB()
  const item = await Topic.findByIdAndUpdate(params.id, { isActive: false }, { new: true })
  if (!item) return NextResponse.json({ ok: false, error: 'Không tìm thấy.' }, { status: 404 })
  return NextResponse.json({ ok: true, message: 'Đã vô hiệu hoá chủ đề.' })
}
