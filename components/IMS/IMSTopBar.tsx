...
const leftNavItems = [
  ...
  { label: 'MẪU BÀI ĐĂNG MXH', href: '#', icon: <ShareIcon /> },
]

const rightNavItems = [
  { label: 'QUẢN LÝ', href: '/dashboard/quan-ly', icon: <GearIcon /> },
  ...
  return (
    <header className="h-12 bg-white border-b border-gray-200 flex items-center px-4 sticky top-0 z-40 shadow-sm">
      ...
      <div className="flex items-center gap-2 ml-4 pl-4 border-l border-gray-200">
        {/* Grid/Apps icon */}
        <button className="grid grid-cols-3 gap-[3px] p-1 hover:bg-gray-100 rounded transition-colors">
          {Array.from({ length: 9 }).map((_, i) => (
            <span key={i} className="w-[5px] h-[5px] bg-gray-400 rounded-[1px] block" />
          ))}
        </button>
